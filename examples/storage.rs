...
// // `TypeMapPanickableStore` implements `TypeMap` store with panics on
// resolves // types.
...
//         // The handler requires `u32` and `String` types from the input
// storage.         Endpoint::by_store(move |num: Arc<u32>, string: Arc<String>|
// async move {             assert_eq!(*num, expected_num);
...
//     // This will cause a panic because we do not store `Ipv4Addr` in out
// store.     let handle = tokio::spawn(async move {
